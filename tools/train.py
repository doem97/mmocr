...
def main():
    # read config from file and command line
    args = parse_args()
    ...
    # build the model, the obj_cls(**args)
    model = build_detector(
        cfg.model,
        train_cfg=cfg.get("train_cfg", None),
        test_cfg=cfg.get("test_cfg", None),
    )
    model.init_weights()  # init model weights

    # build train datawrapper (datawrapper: build multiple dataflows, dataset of dataset, which is also a child of torch.dataset)
    ...
    # add an attribute for visualization convenience; datasets[0] is cfg.data.train
