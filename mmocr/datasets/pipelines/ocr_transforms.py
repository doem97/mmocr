    ...
    def __init__(
        ...
        if not keep_aspect_ratio:
            assert max_width is not None, (
                '"max_width" must assigned ' 'if "keep_aspect_ratio" is False'
            )
        ...
        if self.keep_aspect_ratio:
            ...
            if dst_max_width is not None:
                ...
                img_resize = mmcv.imresize(
                    results["img"], (resize_width, dst_height), backend=self.backend
                )
                ...
                img_resize = mmcv.imresize(
                    results["img"], (new_width, dst_height), backend=self.backend
                )
                resize_shape = img_resize.shape
                pad_shape = img_resize.shape
        else:
            img_resize = mmcv.imresize(
                results["img"], (dst_max_width, dst_height), backend=self.backend
            )
            resize_shape = img_resize.shape
            pad_shape = img_resize.shape
...
class RandomRotateImageBox:
    ...
    def __call__(self, results):
        ...
        random_angle = np.random.uniform(
            max(self.min_angle, -temp_max_angle), min(self.max_angle, temp_max_angle)
        )
        ...
        out_img = TF.rotate(
            in_img, random_angle, resample=False, expand=False, center=rotate_center
        )

        out_boxes, out_chars = self.rotate_bbox(
            in_boxes, in_chars, random_angle_radian, rotate_center, img_box
        )
